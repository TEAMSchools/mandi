...
FROM
    (
     SELECT co.year            
           ...
           /* demographics */      
           ,CONVERT(FLOAT,CASE WHEN co.LUNCHSTATUS IN ('F','R') THEN 1.0 ELSE 0.0 END) * 100 AS is_FR_lunch
           ,CONVERT(FLOAT,CASE WHEN co.SPEDLEP LIKE '%SPED%' THEN 1.0 ELSE 0.0 END) * 100 AS is_SPED      

           /* attrition */
           ,CASE
             WHEN co.year = KIPP_NJ.dbo.fn_Global_Academic_Year() THEN CONVERT(FLOAT,CASE WHEN co.entrydate <= CONVERT(DATE,CONCAT(co.year,'-10-15')) AND co.exitdate <= CONVERT(DATE,GETDATE()) THEN 1.0 ELSE 0.0 END) * 100 
