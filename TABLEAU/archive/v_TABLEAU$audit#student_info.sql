...
ALTER VIEW v_TABLEAU$audit#student_info AS 
...
(
	SELECT schoolid
	  ...
	  ,lastfirst AS detail
	  ,CASE WHEN lastfirst LIKE '%;%' THEN 1
	...
	SELECT schoolid
	  ...
	  ,grade_level
	  ,'Email' AS element
	  ,CASE WHEN guardianemail IS NULL THEN '[Missing]' 
	   ELSE guardianemail END AS detail
	  ,CASE WHEN guardianemail LIKE '%;%' THEN 1
	...
	SELECT schoolid
	  ...
	  ,CASE WHEN MOTHER_CELL LIKE '%;%' THEN 1
	...
	SELECT schoolid
	  ...
	  ,CASE WHEN FATHER_CELL LIKE '%;%' THEN 1
	...
	SELECT schoolid
	  ...
	  ,HOME_PHONE AS detail
	  ,CASE WHEN HOME_PHONE LIKE '%;%' THEN 1
