CREATE OR REPLACE VIEW KIPP_NWK$GRADES_TIME AS
SELECT stu_studentid AS studentid
      ,date_value
      ,course_number
      ,teacher
      ,section_number
      ...
      ,CASE
         --divide by zero...
         WHEN SUM(pts_possible) = 0 THEN NULL
         WHEN SUM(weighting) = 0 THEN NULL
         --total points
         WHEN finalgradesetuptype = 'TotalPoints'
           THEN ROUND((SUM(pts_earned) / SUM(pts_possible)) * 100,2)
         --traditional category weights
         WHEN finalgradesetuptype = 'WeightedFGSetup'
           THEN ROUND((SUM(weighted_pts) / SUM(weighting)) * 100,2)
       END AS synthetic_percent
      ,SUM(num_missing) AS num_missing
      ,SUM(num_missing_or_zero) AS num_missing_or_zero
      ,SUM(num_assign) AS num_assign
      ,NULL AS elements_missing
      ,NULL AS elements_missing_or_zero
      --stragg_vert_pipe is a custom function, can only execute on XE
      /*
      ,stragg_vert_pipe(
         CASE
           WHEN LENGTH(assign_missing) > 0
             THEN '[' || TRIM(cat_abbrev) || '] ' || assign_missing
         END)
       AS elements_missing
      ,stragg_vert_pipe(
         CASE
           WHEN LENGTH(assign_missing_or_zero) > 0
             THEN '[' || TRIM(cat_abbrev) || '] ' || assign_missing_or_zero
         END)
       AS elements_missing_or_zero
       */
FROM   
      --SUB 7 - drop categories with null pts earned (eg no valid scores
      --entered by teacher!)
      --tuning: cost 31
     (SELECT sub_6.*
             ,CASE
                WHEN cat_weight_divisor = 0 THEN 0
                ELSE ROUND((cat_weight_dividend/cat_weight_divisor) * weighting,8)
              END AS weighted_pts
      FROM
            --SUB 6 - group by assignment category, count, tag missing and zeros
            --tuning: cost 31
            (SELECT stu_studentid
                  ,lastfirst
                  ,schoolid
                  ,date_value
                  ,course_number
                  ...
                  ,SUM(CASE
                         --NULL/no score entered don't count in PS gradebook logic
                         WHEN score IS NULL THEN NULL
                         ELSE pointspossible * weight
                       END 
                    ) AS pts_possible
                  ,SUM(score * weight) AS cat_weight_dividend
                  ,SUM(CASE
                         WHEN score IS NULL THEN NULL
                         ELSE pointspossible * weight
                       END) AS cat_weight_divisor
                  --for cat weights
                  --,psm_assignmentscore.exempt
                 ,SUM(
                    CASE
                      WHEN assignment_count_test = 1 THEN ismissing
                    END
                  ) AS num_missing
                 ,SUM(CASE
                        WHEN (assignment_count_test = 1 AND ismissing = 1) THEN 1
                        WHEN (assignment_count_test = 1 AND score = 0) THEN 1
                        ELSE 0
                      END
                  ) AS num_missing_or_zero
                  
                 --stragg_vert_pipe is a custom function
                /* 
                 ,stragg_vert_pipe
                   --what to aggregate
                   (CASE
                      WHEN ismissing = 1 
                        OR score = 0
                        THEN assignment_name
                      ELSE NULL
                    END)
                  AS assign_missing_or_zero
                 ,stragg_vert_pipe
                   --what to aggregate
                   (CASE
                      WHEN ismissing = 1 
                  ...
                  AS assign_missing
                */
                 ,SUM(NVL(assignment_count_test,0)) AS num_assign
            FROM 
                  --SUB 5 - drop the EXCLUDE assignments
                  --(keeping this in one step/subquery eases debuggingg)
                  --tuning: cost 30
                 (SELECT sub_4.*
                        ,CASE
                           WHEN finalgradesetuptype = 'WeightedFGSetup'
