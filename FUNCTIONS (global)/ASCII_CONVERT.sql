  ...
  IF CHARINDEX('&#',@string) > 0
    BEGIN
      ...
      BEGIN    
        ...
        SET @out_string = ISNULL(@out_string,'') + ISNULL(@in_string,'') + ISNULL(@char,''); -- append to results string   
        SET @index = (@ascii_end + 1); -- set index to character after ASCII code    
      END      
      SET @out_string = ISNULL(@out_string,'') + ISNULL(SUBSTRING(@string, @ascii_end + 1, @len - @ascii_end),''); -- when the loop breaks, append the residual string
    END  
  ELSE SET @out_string = @string  
