...
/****** Object:  View [dbo].[ATTRITION$yearly_with_gender_and_MAP]    Script Date: 7/14/2015 11:00:23 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[ATTRITION$yearly_with_gender_and_MAP] AS
...
SELECT CASE GROUPING(sub.school_name)
			      WHEN 1 THEN 'network'
			      ELSE sub.school_name
			    END AS school
			   ,CASE GROUPING(year)
				     WHEN 1 THEN 'all'
				     ELSE 
...
GROUP BY ROLLUP(sub.school_name)
        ,ROLLUP(year)
