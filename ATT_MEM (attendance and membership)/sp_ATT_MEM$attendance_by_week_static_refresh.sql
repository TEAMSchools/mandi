...
ALTER PROCEDURE [dbo].[sp_ATT_MEM$attendance_by_week#static|refresh] AS
BEGIN

 DECLARE @sql AS VARCHAR(MAX)='';

 --STEP 1: make sure no temp table
		IF OBJECT_ID(N'tempdb..#ATT_MEM$attendance_by_week#static|refresh') IS NOT NULL
		BEGIN
						DROP TABLE [#ATT_MEM$attendance_by_week#static|refresh]
		END
		
		--STEP 2: load into a TEMPORARY staging table.
		SELECT *
		INTO [#ATT_MEM$attendance_by_week#static|refresh]
		--FOR CREATING TABLE FIRST TIME
		--INTO ATT_MEM$attendance_by_week#static
    ...
    FROM
       ...
       (SELECT studentid
              ...
              ,CASE
                 ...
                 WHEN att_dense / mem_dense >= .2  THEN 1
                 WHEN att_dense / mem_dense <  .2  THEN 0
                 ELSE NULL
           ...
           (SELECT studentid
            ...
            FROM
                ...
                (SELECT studentid
                      ...
                      ,yearid
                      ,reporting_hash
                      ,MAX(mem_total) AS rolling_mem
                      ,MAX(att_total) AS rolling_att
                FROM
                    ...
                    (SELECT studentid
                          ...
                          ,SUM(att_value) AS att_total
                    FROM 
                       --LEVEL 2: get att status for each day of 
...
')
  ...
  --STEP 4: truncate 
  EXEC('TRUNCATE TABLE dbo.[ATT_MEM$attendance_by_week#static]');
  ...
  WHERE sys.indexes.type_desc = 'NONCLUSTERED'
   ...
   AND sys.objects.name = 'ATT_MEM$attendance_by_week#static';

 EXEC (@sql);

 -- step 3: insert rows from remote source
 INSERT INTO SPI..[ATT_MEM$attendance_by_week#static]
 SELECT *
 FROM [#ATT_MEM$attendance_by_week#static|refresh];
