...
SELECT s.id
      ...
      ,s.schoolid
      ,s.grade_level
      ,SUM(CONVERT(FLOAT,CASE WHEN RT != 'CUR' THEN membershipvalue ELSE 0 END)) AS mem
      ...
      ,SUM(CONVERT(FLOAT,CASE WHEN RT  = 'CUR' THEN membershipvalue ELSE 0 END)) AS CUR_mem
FROM STUDENTS s
LEFT OUTER JOIN
     (SELECT studentid
            ,calendardate
