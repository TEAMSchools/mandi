...
ALTER PROCEDURE [dbo].[sp_ATT_MEM$tardiness_by_week_unbounded#static|refresh] AS
BEGIN

 DECLARE @sql AS VARCHAR(MAX)='';

 --STEP 1: make sure no temp table
		IF OBJECT_ID(N'tempdb..#ATT_MEM$tardiness_by_week_unbounded#static|refresh') IS NOT NULL
		BEGIN
						DROP TABLE [#ATT_MEM$tardiness_by_week_unbounded#static|refresh]
		END
...
')
  ...
  --STEP 4: truncate 
  EXEC('TRUNCATE TABLE dbo.[ATT_MEM$tardiness_by_week_unbounded#static]');
  ...
  WHERE sys.indexes.type_desc = 'NONCLUSTERED'
   ...
   AND sys.objects.name = 'ATT_MEM$tardiness_by_week_unbounded#static';

 EXEC (@sql);

 -- step 3: insert rows from remote source
 INSERT INTO SPI..[ATT_MEM$tardiness_by_week_unbounded#static]
 SELECT *
 FROM [#ATT_MEM$tardiness_by_week_unbounded#static|refresh];
