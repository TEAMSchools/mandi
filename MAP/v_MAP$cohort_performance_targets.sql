...
,cohort AS (
  SELECT c.studentid
        ...
        ,c.year
        ,CASE 
           WHEN c.spedlep LIKE 'SPED%' THEN 'IEP'
           ELSE 'Gen Ed'
         END AS iep_status
...
FROM
    (
     ...
     SELECT sub.*
           ...
           ,cohort_norms.sd
           ,ROW_NUMBER() OVER(
              PARTITION BY sub.school, sub.grade_level, sub.year, sub.iep_status, sub.measurementscale
                  ORDER BY ABS(avg_baseline_rit - cohort_norms.rit)) AS rn
     FROM
         (
          SELECT sub.school
                ,sub.grade_level
                ,sub.year
                ,CASE GROUPING(sub.iep_status) WHEN 1 THEN 'All Students' ELSE sub.iep_status END AS iep_status
                ,sub.measurementscale
