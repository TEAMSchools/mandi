...
WITH global_min_tested AS
   ...
   (SELECT m.measurementscale
          ...
          ,COUNT(*) AS n
    FROM KIPP_NJ..MAP$comprehensive#identifiers m
    JOIN KIPP_NJ..MAP$comprehensive#identifiers m_next
   ...
   (SELECT m.measurementscale
          ...
          ,COUNT(*) AS n
    FROM KIPP_NJ..MAP$comprehensive#identifiers m
    JOIN KIPP_NJ..MAP$comprehensive#identifiers m_prev
...
FROM
      (SELECT sub.*
             ,CASE
                WHEN npr_behind_z < 0 AND npr_ahead_z < 0 THEN npr_behind_z + npr_ahead_z ELSE NULL
              END AS v_dip_signature_z
       FROM
             (SELECT m.*
