...
SELECT adp.associate_id
      ...
      ,adp.rn_base
	  ,adp.rn_curr
	  ,CASE WHEN gdoc.firstlast IS NULL THEN 1 ELSE 0 END AS update_status
	  ...
	  ,CASE WHEN gdoc.firstlast IS NULL THEN adp.first_name + ' ' + adp.last_name ELSE gdoc.firstlast END AS combined_name
...
*/
	  ...
	  ,CONCAT(CASE WHEN gdoc.grade_k_teacher IS NOT NULL THEN 'K,' END
  	  ...
  	  ,adp.department
	  ,CASE WHEN gdoc.subject_primary IS NULL THEN adp.department ELSE gdoc.subject_primary END AS subject_department

	  ,adp.job_title
	  ,CASE WHEN gdoc.job_title IS NULL THEN adp.job_title ELSE gdoc.job_title END AS gdoc_job_title
...
WHERE adp.rn_curr = 1
AND adp.position_status = 'Active'
